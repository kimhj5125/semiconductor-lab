...
with col_left:
    ...
    if region == "Saturation":
        ...
        if device == "NMOS":
            ...
            dep_rect = patches.Rectangle((po_x, SIO2_BOTTOM - CH_THICK),
                                         GATE_X_END - po_x, CH_THICK,
            ...
            dep_rect = patches.Rectangle((GATE_X_START, SIO2_BOTTOM - CH_THICK),
                                         po_x - GATE_X_START, CH_THICK,
    ...
    ax.text(5, 7.8,
            f"Applied: V_GS={vgs:.1f}V | V_DS={vds:.1f}V",
...
with col_mid:
    ...
    ax_iv.plot(vds_space, id_curve,
               color="#1a5276", lw=2.5, label=f"V_GS = {vgs:.1f} V")
    ...
    ax_iv.plot(sat_vds_pts, sat_id_pts,
               color="#e74c3c", lw=1.8, linestyle='--',
               label="Saturation Boundary (V_DS = V_GS - V_TH)")

    # 동작점
    ax_iv.plot(vds, id_mA, 'ro', ms=9, zorder=8,
    ...
    ax_b.axhline(ef_level, xmin=0 / 3.6, xmax=0.8 / 3.6,
                 color='purple', lw=1.3, linestyle=':', alpha=0.8)
    ax_b.axhline(ef_level, xmin=2.8 / 3.6, xmax=3.6 / 3.6,
                 color='purple', lw=1.3, linestyle=':', alpha=0.8, label="$E_F$")

    ax_b.annotate("", xy=(0.3, ec_src_arr[0]), xytext=(0.3, ev_src_arr[0]),
                  arrowprops=dict(arrowstyle='<->', color='gray', lw=1.0))
    ax_b.text(0.35, (ec_src_arr[0] + ev_src_arr[0]) / 2, f"Eg={Eg}eV",
